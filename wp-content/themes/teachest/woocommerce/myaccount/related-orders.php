		...
		<?php foreach ( $subscription_orders as $subscription_order ) {
            ...
            echo '<div class="text-right">';
						foreach ( $actions as $key => $action ) {
							echo wp_kses_post( '<a href="' . esc_url( $action['url'] ) . '" class="btn btn-tc-default btn-sm text-uppercase ' . sanitize_html_class( $key ) . '" style="margin-right:5px">' . esc_html( $action['name'] ) . '</a>' );
						}
            echo '</div>';
					}
					?>
